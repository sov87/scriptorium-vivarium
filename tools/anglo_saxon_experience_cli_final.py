...
Stateful Anglo-Saxon Historical Simulation Engine
...
# Runtime world context for world-aware validation and prompts.
...
def print_help_ui(world_config: Dict[str, Any]) -> None:
    ...
    print("  > head to the minster")
    print("  The engine resolves names via semantic aliases (e.g. 'church' -> 'minster').")
    print("  If you name a distant city, the engine uses Haversine math to travel there.")
    
    print("\n[INTERACTION & PHYSICS]")
    print("  > talk to the merchant")
    print("  > ask the reeve about dues")
...
def graph_node_match_confidence(target: str, node_key: str, node_data: Dict[str, Any]) -> float:
    target_tokens = [x for x in re.findall(r"[a-z0-9]+", (target or "").lower()) if len(x) >= 2 and x not in STOPWORDS]
    if not target_tokens:
        return 0.0

    candidates: List[str] = [node_key]
    if isinstance(node_data, dict):
        candidates.extend([str(a) for a in node_data.get("aliases", []) if isinstance(a, str)])

    best = 0.0
    tset = set(target_tokens)
    for c in candidates:
        cset = set([x for x in re.findall(r"[a-z0-9]+", c.lower()) if len(x) >= 2])
        if not cset:
            continue
        overlap = len(tset & cset) / max(1, len(tset))
        phrase = 0.2 if (target.lower() in c.lower() or c.lower() in target.lower()) else 0.0
...
def retrieve_packet_with_quotas(
    ...
    if loc_hit_count == 0 and loc_label.lower() in world.get("graph", {}):
        ...
        if neighbor_names:
            ...
            all_hits.extend(store.search_fts(neighbor_q, corpora_filter if corpora_filter else None, limit=18))

    # Adaptive backfill for sparse corpora: widen with life/pol/core terms when recall is weak.
    weak_recall = len(all_hits) < 14 or loc_hit_count == 0
    if weak_recall:
        adaptive_terms = list(world.get("core_keywords", []))[:6] + list(world.get("life_terms", []))[:8] + list(world.get("pol_terms", []))[:6]
        adaptive_terms += btoks[:6] + time_boost_terms[:3] + weather_boost_terms[:3]
        adaptive_terms = [t for t in adaptive_terms if isinstance(t, str) and t.strip()]
        if adaptive_terms:
            aq = build_or_query(adaptive_terms)
            if aq:
                all_hits.extend(store.search_fts(aq, corpora_filter if corpora_filter else None, limit=16))
    ...
    unique_corpora = len(set([h.get("corpus_id", "") for h in packet if h.get("corpus_id")]))
    meta = {
        "scope": scope,
        "loc_label": loc_label,
        "intent_tokens": itoks,
        ...
        "loc_hit_count": loc_hit_count,
        "weak_recall": weak_recall,
        "packet_corpus_diversity": unique_corpora,
    }
...
def sentence_has_specific_claim(sentence: str) -> bool:
    ...
    world = CURRENT_WORLD_CONTEXT if isinstance(CURRENT_WORLD_CONTEXT, dict) else {}
    entity_re = build_specific_entity_re(world)

    # Only treat place-name mentions as hard claims when coupled with assertive factual phrasing.
    if entity_re.search(s) and ASSERTIVE_FACT_RE.search(s):
        return True
...
def extract_hard_claim_anchors(sentence: str) -> List[str]:
    world = CURRENT_WORLD_CONTEXT if isinstance(CURRENT_WORLD_CONTEXT, dict) else {}
    return extract_hard_claim_anchors_world(sentence, world)
...
def sentence_supported_by_evidence(sentence: str, packet_map: Dict[str, str]) -> bool:
    ...
    world = CURRENT_WORLD_CONTEXT if isinstance(CURRENT_WORLD_CONTEXT, dict) else {}
    dynamic_stop = support_stopwords_for_world(world)
    sent_tokens = [
        tok for tok in re.findall(r"[a-z0-9]+", s_clean)
        if len(tok) >= 4 and tok not in dynamic_stop
    ]
    ...
    if build_specific_entity_re(world).search(sentence or "") and ASSERTIVE_FACT_RE.search(sentence or ""):
        return max_overlap >= 2
...
def world_text_tokens(world: Dict[str, Any]) -> Set[str]:
    toks: Set[str] = set()
    if not isinstance(world, dict):
        return toks

    for k in ("core_keywords", "life_terms", "pol_terms"):
        for term in world.get(k, []) if isinstance(world.get(k), list) else []:
            for tok in re.findall(r"[a-z0-9]+", str(term).lower()):
                if len(tok) >= 3:
                    toks.add(tok)

    for node_key, node_data in (world.get("graph", {}) or {}).items():
        for tok in re.findall(r"[a-z0-9]+", str(node_key).lower()):
            if len(tok) >= 3:
                toks.add(tok)
        if isinstance(node_data, dict):
            for alias in node_data.get("aliases", []) or []:
                for tok in re.findall(r"[a-z0-9]+", str(alias).lower()):
                    if len(tok) >= 3:
                        toks.add(tok)

    for era_cfg in (world.get("eras", {}) or {}).values():
        if not isinstance(era_cfg, dict):
            continue
        for bg in era_cfg.get("bias_groups", []) or []:
            if not isinstance(bg, dict):
                continue
            for key in ("triggers", "terms"):
                for term in bg.get(key, []) or []:
                    for tok in re.findall(r"[a-z0-9]+", str(term).lower()):
                        if len(tok) >= 3:
                            toks.add(tok)
    return toks
...
def build_specific_entity_re(world: Dict[str, Any]) -> re.Pattern:
    tokens = sorted(world_text_tokens(world))
    if not tokens:
        return re.compile(r"$^", re.IGNORECASE)
    escaped = [re.escape(t) for t in tokens if len(t) >= 4]
    if not escaped:
        return re.compile(r"$^", re.IGNORECASE)
    return re.compile(r"\b(?:" + "|".join(escaped[:220]) + r")\b", re.IGNORECASE)


def support_stopwords_for_world(world: Dict[str, Any]) -> Set[str]:
    base = set(SUPPORT_STOPWORDS)
    base.update({"market", "street", "crowd", "vendor", "vendors", "people", "city", "town"})
...
def anchor_blocklist_for_world(world: Dict[str, Any]) -> Set[str]:
    out = set(HARD_ANCHOR_NAME_BLOCKLIST)
    for term in world.get("core_keywords", []) if isinstance(world.get("core_keywords"), list) else []:
        for tok in re.findall(r"[a-z0-9]+", str(term).lower()):
            out.add(tok)
    return out


def extract_hard_claim_anchors_world(sentence: str, world: Dict[str, Any]) -> List[str]:
    s = sentence or ""
    anchors: List[str] = []
    ...
    world_pat = build_specific_entity_re(world)
    for m in world_pat.findall(s):
        if isinstance(m, str):
            anchors.append(m.lower())
        elif isinstance(m, tuple):
            anchors.extend([x.lower() for x in m if x])

    blocked = anchor_blocklist_for_world(world)
    for m in re.findall(r"\b[A-Z][a-z]{2,}\b", s):
        ml = m.lower()
        if ml not in blocked:
            anchors.append(ml)
...
def localize_prompt_text(text: str, world: Dict[str, Any], era_config: Dict[str, Any]) -> str:
    wname = str(world.get("setting_label", world.get("name", "the setting")))
    people = str(world.get("people_label", "people"))
    singular = people[:-1] if people.endswith("s") else people

    out = text or ""
    out = out.replace("{setting_label}", wname)
    out = out.replace("{people_label}", people)
    out = out.replace("{people_label_singular}", singular)
    out = out.replace("ancient Rome", wname)
    out = out.replace("Romans", people)
    out = out.replace("Roman", singular)
    out = out.replace("{system_context}", era_config.get("system_context", ""))
    out = out.replace("{architectural_notes}", era_config.get("architectural_notes", ""))
    return out

def build_system_prompt(world: Dict[str, Any], era_config: Dict[str, Any], running_summary: str) -> str:
    text = localize_prompt_text(PROMPTS_DATA["system_prompt"], world, era_config)
...
def build_render_prompt(world: Dict[str, Any], era_config: Dict[str, Any], state: Dict[str, Any], history: List[Dict[str, str]], plan: Dict[str, Any], packet: List[Dict[str, str]]) -> str:
    d = copy.deepcopy(PROMPTS_DATA["render_prompt"])
    d["task"] = localize_prompt_text(str(d.get("task", "")), world, era_config)
    reqs = d.get("requirements", [])
    if isinstance(reqs, list):
        d["requirements"] = [localize_prompt_text(str(r), world, era_config) for r in reqs]
    d["player_state"] = state
    d["recent_history"] = history
    d["engine_plan"] = plan
    d["evidence_packet"] = [{"segment_id": p["segment_id"], "text": p["excerpt"]} for p in packet]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Vivarium: Anglo-Saxon Stateful Historical Simulation Engine")
    ...
    ap.add_argument("--world", default="anglo_saxon", choices=list(WORLDS.keys()), help="Select the simulated world setting.")
    ...
    CURRENT_WORLD_CONTEXT = world_config
        ...
        while True:
            ...
            if user_in.startswith(":"):
                ...
                if cmd == "suggest":
                    ...
                    msgs = [
                        {"role": "system", "content": build_system_prompt(world_config, era_config, state.get("running_summary", ""))},
            ...
            try:
                ...
                if meta_access_requested:
                    ...
                    system_constraint = (system_constraint + " " if system_constraint else "") + (
                        ...
                        "Permit ordinary interaction immediately in a procedurally plausible historical setting without fantasy framing."
                    )
                ...
                if m_goto:
                    ...
                    for node_key, node_data in world_graph.items():
                        score = graph_node_match_confidence(target_raw, node_key, node_data if isinstance(node_data, dict) else {})
                        if score > 0:
                            ranked_nodes.append((score, node_key))
                    ranked_nodes.sort(reverse=True)

                    best_match = ranked_nodes[0][1] if ranked_nodes else None
                    best_score = ranked_nodes[0][0] if ranked_nodes else 0.0
                    second_score = ranked_nodes[1][0] if len(ranked_nodes) > 1 else 0.0

                    if best_match and best_score >= 0.55 and (best_score - second_score >= 0.10 or best_score >= 0.80):
                        ...
                        if node_era_reqs and state["era"] not in node_era_reqs:
                            ...
                            top_opts = [n.title() for _, n in ranked_nodes[:3]]
                            system_constraint = f"Player tried to go to {best_match_title}, but it is not directly connected to {current_loc.title()}. Offer nearby alternatives: {', '.join(top_opts)}."
                    else:
                        gaz_id = world_config.get("gazetteer_corpus_id")
                        if gaz_id:
                            ...
                            if hits:
                                ...
                                if conf0 >= 0.68 and (conf0 - conf1) >= 0.15:
                                    ...
                                    choices = [parse_gazetteer_title(h["excerpt"]) for h in hits[:3] if parse_gazetteer_title(h["excerpt"])]
                                    system_constraint = f"Player destination '{target_raw.title()}' is ambiguous. Offer clarifying options: {', '.join(choices) if choices else 'nearest known settlements'} while keeping the player at {current_loc.title()}."
                            else:
                                system_constraint = f"No exact destination found for '{target_raw.title()}'. Keep player at {current_loc.title()} and render local exploratory movement with concrete options nearby."
                        else:
                            system_constraint = f"No mapped destination found for '{target_raw.title()}'. Keep player local and render a grounded exploratory walk with practical next places to try."
                ...
                for attempt in range(1, max_render_attempts + 1):
                    ...
                    msgs = [
                        {"role": "system", "content": build_system_prompt(world_config, era_config, state.get("running_summary", ""))},
                        {"role": "user", "content": "/no_think\n" + build_render_prompt(world_config, era_config, state, list(history), plan_obj, packet)},
                    ]
                    if attempt > 1:
